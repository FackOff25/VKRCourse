...
public:
    ...
    Path<KeyType> find_path(const NodeKey<KeyType>& start, const NodeKey<KeyType>& goal) override {
        ...
        int max_hops = 25;
        int inter_storage_edges = 0;   // ← счётчик переходов между хранилищами
        ...
        for (int hop = 0; hop < max_hops; ++hop) {
            ...
            // Переход в другое хранилище
            current = res.exit_node;
            inter_storage_edges++;          // ← каждый такой переход — ребро между хранилищами
